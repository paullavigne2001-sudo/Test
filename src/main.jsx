...
function Game(){
 ...
 const updateJoy=e=>{e.preventDefault();const r=e.currentTarget.getBoundingClientRect(),x=clamp((e.clientX-(r.left+r.width/2))/(r.width*.42),-1,1),y=clamp((e.clientY-(r.top+r.height/2))/(r.height*.42),-1,1);joy.current.x=x;joy.current.y=y;setJoyUi({x,y})};
 const startJoy=e=>{e.preventDefault();joyActive.current=true;e.currentTarget.setPointerCapture(e.pointerId);updateJoy(e)};const moveJoy=e=>{if(joyActive.current)updateJoy(e)};const endJoy=e=>{if(e)e.preventDefault();joyActive.current=false;joy.current.x=0;joy.current.y=0;setJoyUi({x:0,y:0})};
 ...
 <div className="controls"><div className="joystick" onPointerDown={startJoy} onPointerMove={moveJoy} onPointerUp={endJoy} onPointerCancel={endJoy}><div className="knob" style={{transform:`translate(${joyUi.x*28}px,${joyUi.y*28}px)`}}/></div><button className="dodge" onPointerDown={e=>{e.preventDefault();e.stopPropagation();dodge()}}>↪</button><button className={"attack "+(attackTimer.current>0?"active":"")} onPointerDown={e=>{e.preventDefault();e.stopPropagation();attack()}}>👊</button></div>
