...
function Game(){
 ...
 useEffect(()=>{spawn(1)},[]);
 const attack=()=>{if(paused||gameOver||victory||attackTimer.current>0||hp<=0)return;attackTimer.current=.34;let target=null,best=2.8;for(const e of enemies.current){const d=dist(player.current,e);if(e.hp>0&&d<best){best=d;target=e}}if(target){const dx=target.x-player.current.x,dz=target.z-player.current.z;target.hp=Math.max(0,target.hp-(level>=3?20:25));target.x+=dx/(dist(player.current,target)||1)*.45;target.z+=dz/(dist(player.current,target)||1)*.45;target.attack=0;hitFlash.current=.18;setCombo(c=>c+1);setScore(s=>s+25);setEnemyTick(t=>t+1);if(target.hp<=0)setScore(s=>s+(target.boss?250:75))}else setCombo(0)};
