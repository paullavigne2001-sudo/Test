...
function Arena({level}){
 return <><mesh rotation={[-Math.PI/2,0,0]} receiveShadow><planeGeometry args={[30,30]}/><meshStandardMaterial color={level>=3?'#24151c':'#172033'}/></mesh><gridHelper args={[30,30,level>=3?'#7f1d1d':'#334155',level>=3?'#451a1a':'#1e293b']}/><mesh position={[0,2,-8]}><boxGeometry args={[16,4,.4]}/><meshStandardMaterial color="#111827"/></mesh><mesh position={[-8,2,0]}><boxGeometry args={[.35,4,16]}/><meshStandardMaterial color="#111827"/></mesh><mesh position={[8,2,0]}><boxGeometry args={[.35,4,16]}/><meshStandardMaterial color="#111827"/></mesh><mesh position={[0,.05,-3]} rotation={[-Math.PI/2,0,0]}><circleGeometry args={[2.8,48]}/><meshStandardMaterial color={level>=3?'#4c1d95':'#1e3a5f'} emissive={level>=3?'#7c3aed':'#0ea5e9'} emissiveIntensity={.35}/></mesh></>
}
...
function Game(){
 ...
 <Canvas shadows dpr={[1,1.5]} camera={{position:[0,4.4,7.1],fov:55}}><color attach="background" args={['#080d18']}/><ambientLight intensity={1.2}/><directionalLight position={[5,9,4]} intensity={3.2} castShadow/><pointLight position={[-5,3,1]} intensity={18} distance={12}/><pointLight position={[5,2,-4]} intensity={14} distance={10} color="#7c3aed"/><Arena level={level}/><Fighter fighter={player} player enemy attack={attackTimer.current/.34} hit={hitFlash.current} enemyHp={0}/>{alive.map((e,i)=><Fighter key={i} fighter={{current:e}} player={player} enemy={e} attack={e.attack>0?1-e.attack/.9:0} hit={0} enemyHp={e.hp/e.maxHp*100} boss={e.boss}/>)}<CameraRig player={player} level={level}/></Canvas>
