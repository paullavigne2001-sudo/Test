...
function Game(){
 ...
 const keys=useRef({}),joy=useRef({x:0,y:0}),joyActive=useRef(false),attackTimer=useRef(0),dodgeTimer=useRef(0),invuln=useRef(0),hitFlash=useRef(0);
 ...
 useEffect(()=>{spawn(1)},[]);
 const attack=()=>{if(paused||gameOver||victory||attackTimer.current>0||hp<=0)return;attackTimer.current=.34;let target=null,best=2.15;for(const e of enemies.current){const d=dist(player.current,e);if(e.hp>0&&d<best){best=d;target=e}}if(target){const dx=target.x-player.current.x,dz=target.z-player.current.z;target.hp=Math.max(0,target.hp-(level>=3?20:25));target.x+=dx/(dist(player.current,target)||1)*.45;target.z+=dz/(dist(player.current,target)||1)*.45;target.attack=0;hitFlash.current=.18;setCombo(c=>c+1);setScore(s=>s+25);setEnemyTick(t=>t+1);if(target.hp<=0)setScore(s=>s+(target.boss?250:75))}else setCombo(0)};
 const dodge=()=>{if(stamina<25||dodgeTimer.current>0||gameOver||victory)return;let x=joy.current.x,y=joy.current.y;if(Math.hypot(x,y)<.1){x=Math.sin(player.current.angle);y=Math.cos(player.current.angle)}const l=Math.hypot(x,y)||1;player.current.x=clamp(player.current.x+x/l*1.15,-7,7);player.current.z=clamp(player.current.z+y/l*1.15,-6,7);staminaRef.current=25;dodgeTimer.current=.5;invuln.current=.5};
 ...
 if(alive===0&&!changed){changed=true;if(level<3){const nl=level+1;setLevel(nl);setScore(s=>s+100);setTimeout(()=>spawn(nl),650)}else{setVictory(true);setScore(s=>s+500)}}
